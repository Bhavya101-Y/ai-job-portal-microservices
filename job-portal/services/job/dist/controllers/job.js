...
export const createCompany = TryCatch(async (req, res) => {
    ...
    try {
        ...
        throw new ErrorHandler(error?.response?.status || 500, error?.response?.data?.message || "Logo upload failed. Please ensure Utils Service is running.");
    }
    const [newCompany] = await sql `INSERT INTO companies (name, description, website, logo, logo_public_id, recruiter_id) VALUES (${name}, ${description}, ${website}, ${uploadResult.url}, ${uploadResult.public_id}, ${user.user_id}) RETURNING *`;
    res.json({
...
export const getMyJobs = TryCatch(async (req, res) => {
    const user = req.user;
    if (!user) {
        throw new ErrorHandler(401, "Authentication required");
    }
